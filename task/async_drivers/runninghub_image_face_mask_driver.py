"""
RunningHub 图片人脸遮盖驱动
使用 RunningHub OpenAPI v2 异步生成已遮盖人脸的图片。
"""
import logging
import traceback
...
from config.constant import RunningHubImageFaceMaskConstants
from utils.file_storage import RunningHubFileStorage

logger = logging.getLogger(__name__)


class RunningHubImageFaceMaskConfig:
    """RunningHub 图片人脸遮盖配置常量"""
    APP_ID = RunningHubImageFaceMaskConstants.APP_ID
    IMAGE_NODE_ID = RunningHubImageFaceMaskConstants.IMAGE_NODE_ID
    IMAGE_FIELD_NAME = RunningHubImageFaceMaskConstants.IMAGE_FIELD_NAME
    FINAL_STATUSES = RunningHubImageFaceMaskConstants.FINAL_STATUSES


class RunningHubImageFaceMaskDriver(BaseAsyncDriver):
    """RunningHub 图片人脸遮盖驱动"""

    @property
    def impl_id(self) -> int:
        from config.unified_config import AsyncTaskImplementationId
        return AsyncTaskImplementationId.RUNNINGHUB_IMAGE_FACE_MASK

    def __init__(self):
        super().__init__("runninghub_image_face_mask")
    ...
    async def submit_task(self, image_path: str, **kwargs) -> Dict[str, Any]:
        """
        提交图片人脸遮盖任务到 RunningHub。

        Args:
            image_path: 本地文件路径或 URL
        """
        try:
            ...
            node_info_list = [
                {
                    'nodeId': RunningHubImageFaceMaskConfig.IMAGE_NODE_ID,
                    'fieldName': RunningHubImageFaceMaskConfig.IMAGE_FIELD_NAME,
            ...
            submit_response = await self._client.run_ai_app_v2(
                app_id=RunningHubImageFaceMaskConfig.APP_ID,
    ...
    async def check_status(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if remote_status in RunningHubImageFaceMaskConfig.FINAL_STATUSES:
                error_message = (
                    query_response.get('errorMessage')
                    or str(query_response.get('failedReason') or '图片人脸遮盖生成失败')
                )
                return {
                    'status': 'FAILED',
                    'error': error_message,
            ...
            logger.error(f"RunningHub image face mask check connection error: {e}")
            return {
                'status': 'FAILED',
                'error': '网络连接异常，请稍后重试',
                'error_type': 'USER',
            ...
            logger.error(f"RunningHub image face mask check timeout: {e}")
            return {
                'status': 'FAILED',
                'error': '请求超时，请稍后重试',
                'error_type': 'USER',
            ...
            logger.error(f"RunningHub image face mask check error: {e}\n{traceback.format_exc()}")
            return {
                'status': 'FAILED',
                'error': f'查询图片人脸遮盖状态失败: {str(e)}',
                'error_type': 'SYSTEM',
    ...
    def _handle_submit_error(e: Exception) -> Dict[str, Any]:
        ...
        if isinstance(e, ConnectionError):
            logger.error(f"RunningHub image face mask submit connection error: {e}")
            return {
                'success': False,
                'error': '网络连接异常，请稍后重试',
                'error_type': 'USER',
        ...
        if isinstance(e, TimeoutError):
            logger.error(f"RunningHub image face mask submit timeout: {e}")
            return {
                'success': False,
                'error': '请求超时，请稍后重试',
                'error_type': 'USER',
        ...
        logger.error(f"RunningHub image face mask submit error: {e}\n{traceback.format_exc()}")
        return {
            'success': False,
            'error': f'提交图片人脸遮盖任务失败: {str(e)}',
            'error_type': 'SYSTEM',
            'error_detail': traceback.format_exc(),
            'retry': False
        }
